...
const API_BASE_DETAILS = 'http://127.0.0.1:3000';
...
async function fetchRecipe(id) {
  const res = await fetch(`${API_BASE_DETAILS}/recipes/${encodeURIComponent(id)}`);
  if (!res.ok) throw new Error('Rezept nicht gefunden');
  return await res.json();
}

function guessImageDetails(name) {
...
function applyData(r) {
  ...
  img.src = r.image || guessImageDetails(r.name);
  img.alt = r.name;
